...
(function ($) {
    $.extend({
    	...
    	modal: {
            ...
            //自定义弹出层全屏
            openCustomFull: function (title, url, width, height) {
            	//如果是移动端，就使用自适应大小弹窗
            	if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            	    width = 'auto';
            	    height = 'auto';
        ...
        operate: {
         ...
         // 审批信息 全屏
            wfViewFull: function(id) {
            	var url = $.table._option.wfViewUrl.replace("{id}", id);
            	$.modal.openFull("审批" + $.table._option.modalName, url);
            },
            changeFull: function(id) {
            	var url = $.table._option.changeUrl.replace("{id}", id);
            	$.modal.openFull("变更" + $.table._option.modalName, url);
            },
            // 修改访问地址
            editUrl: function(id) {
            	var url = "/404.html";
